...
# virtual address region : log details
superpage_tracker = {}

"""
Input:
- super_va: A string representing the related key in the superpage_tracker dictionary
...
def add_instance(super_va, operation_words):
    first_inst = superpage_tracker[super_va].find(operation_words)
    if first_inst != -1:
        x = superpage_tracker[super_va].find("x", first_inst)
        times = int(superpage_tracker[super_va][first_inst+len(operation_words)+1: x])
        superpage_tracker[super_va] = superpage_tracker[super_va][0:first_inst+len(operation_words)+1]\
            + str(times+1) + superpage_tracker[super_va][x:]
        return 0
    else: 
        return -1
    ...
    while True:
        for line in f:
            ...
            if operation == "pmap_enter_pde" or operation == "pmap_demote_pde" or operation == "pmap_promote_pde":
                ...
                if super_va not in superpage_tracker:
                    superpage_tracker[super_va] = ""
                ...
                if line.find("protect") != -1:
                    ...
                    if add_instance(super_va, "Protection added") == -1:
                        superpage_tracker[super_va] += "Protection added 1x. "
                ...
                # Check if operation was successful.
                    ...
                    if operation == "pmap_enter_pde":
                        if add_instance(super_va, "entered") == -1:
                            superpage_tracker[super_va] += "Superpage entered 1x. "
                    
                    elif operation == "pmap_promote_pde":
                        if add_instance(super_va, "promoted") == -1:
                            superpage_tracker[super_va] += "Superpage promoted 1x. "
                    
                    elif operation == "pmap_demote_pde":
                        if add_instance(super_va, "demoted") == -1:
                            superpage_tracker[super_va] += "Superpage demoted 1x. "
                ...
                # Check if operation was failure.
                    ...
                    if operation == "pmap_promote_pde":
                        if add_instance(super_va, "promotion failed") == -1:          
                            superpage_tracker[super_va] += "Superpage promotion failed 1x. "
                    
                    elif operation == "pmap_demote_pde":
                        if add_instance(super_va, "demotion failed") == -1:
                            superpage_tracker[super_va] += "Superpage demotion failed 1x. "
                
            # Testing:
            # test_1000 -= 1
            # if test_1000 < 0:
            #     for spage, log in superpage_tracker.items():
            #         print(spage, ": ", log, sep="") 
